...
def test_load():
    ...
    assert dm.load(0, DataMemory.WORD) == sign_extend(0x04030201)
    assert dm.load(4, DataMemory.HALF_WORD) == sign_extend(0x0504)
    assert dm.load(6, DataMemory.BYTE) == sign_extend(0x07)
    dm.set_enable(False, False)
    assert dm.load(0, DataMemory.WORD) == 'Read Enable is unset!'
...
def test_store():
    ...
    assert dm.store(4, 0x0504, DataMemory.HALF_WORD) == 0x0504
    assert dm.data[4:6] == [5, 4]
    assert dm.store(6, 0x07, DataMemory.BYTE) == 0x07
    assert dm.data[6] == 0x07
    dm.set_enable(False, False)
